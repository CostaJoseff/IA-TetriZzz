...
class Pecas:
  ...
  def model_ajust(self):
    self.peca_atual_modelo = self.pecas[self.id - 1].copy()
    for l in range(len(self.peca_atual_modelo)):
      for c in range(len(self.peca_atual_modelo[0])):
        self.peca_atual_modelo[l][c] = 0 if self.peca_atual_modelo[l][c] == 0 else 99
